...
<body>
   ...
   <h1><p class = "text-center" style = "color:#004DBF; font-size:48px;" id = "percent"></p></h1>
    ...
    <div class = "container">
    <div class = "row-fluid" >
    <p class = "text-center"><a href = "/halffull/3P/"><button type = "submit" id = "submitButton" class = "btn btn-primary" style = "color:#FFFFFF; background:#004DBF"><h1>Next</h1></button></a></p>
    
</body>

<script>
var trapezoidArea = 0.5*(100+190)*800;
var paper = new Raphael(document.getElementById('display'), 800, 220);
var bkTriangle = paper.path('M 0 200 L 800 200 L 800 10 L 0 100 z').attr({fill:"#B5C1E8", stroke:0});

var colorTriangle = paper.path('M 0 100 200 L 0 200 L 0 10 L 0 100 z').attr({fill:"#004DBF",stroke:0});
$(function() {
		$( "#colorSlider").slider({
...
function processData(response){
		...
		var responsePosition = response[i][1]*8;
...
function showValue(){
var sliderValue = $( "#colorSlider" ).slider( "value" );
var newPath = 'M 0 200 L ' + (sliderValue) + ' 200 L ' + (sliderValue) + ' ' + (100-90*sliderValue/800) + ' L 0 100 z';
var percent = 0.5*sliderValue*(200+90*sliderValue/800)*100/trapezoidArea;
...
colorTriangle.attr({path:newPath});
var sliderValue = $( "#colorSlider" ).slider( "value" );
var percent = 0.5*sliderValue*(100+190*sliderValue/800)*100/trapezoidArea;
var divP = document.getElementById('percent')
divP.innerHTML=Math.round(percent) + '%';

}
function drawYourValue(){
myRect = localStorage.getItem("trapezoid")*8;
